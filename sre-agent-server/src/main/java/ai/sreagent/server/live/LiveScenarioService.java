 ...
 * 2. Collect live evidence from Prometheus / Loki / Jaeger
...
public class LiveScenarioService {
    ...
    public LiveScenarioResult runScenarioG(String mode, String faultMode,
        ...
        log.info("Starting Scenario G: mode={}, faultMode={}, wait={}s, llm={}",
                mode, faultMode, waitSeconds, runLlmProposal);
        ...
        try {
            ...
            if ("live".equals(mode)) {
                ...
                if (waitSeconds > 0) {
                    log.info("Waiting {}s for metrics propagation...", waitSeconds);
                    Thread.sleep(waitSeconds * 1000L);
                }
            }

            // Phase 2: Collect evidence
            boolean forceFixture = "simulation".equals(mode);
            LiveEvidenceCollector collector = new LiveEvidenceCollector(
                    prometheusUrl, lokiUrl, jaegerUrl, forceFixture);
            ...
            LiveEvidenceReport mergedReport = new LiveEvidenceReport(
                    allEvidence.size(), allEvidence, mergedSources, allWarnings);

            if (allEvidence.isEmpty()) {
                log.warn("No evidence collected — using fallback static evidence");
