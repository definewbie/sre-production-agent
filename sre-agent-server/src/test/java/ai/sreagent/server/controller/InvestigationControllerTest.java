...
class InvestigationControllerTest {
    ...
    void postScenarioEReturns200() throws Exception {
        mockMvc.perform(post("/api/investigations/scenario-e"))
                ...
                .andExpect(jsonPath("$.confidenceScore").value(closeTo(0.50, 0.01)))
                .andExpect(jsonPath("$.scores.hyp_deployment_regression").value(closeTo(0.50, 0.01)))
                .andExpect(jsonPath("$.scores.hyp_downstream_dependency_latency").value(closeTo(0.45, 0.01)))
